...
{
	this->temp_address = this->bus.cpu_read(this->PC) | (this->bus.cpu_read(this->PC + 1) << 8);
...
{
	this->temp_address = (this->bus.cpu_read(this->PC) | (this->bus.cpu_read(this->PC + 1) << 8)) + this->X;
	this->temp_value   = this->bus.cpu_read(this->temp_address);
	this->PC += 2;
}
...
{
	this->temp_address = (this->bus.cpu_read(this->PC) | (this->bus.cpu_read(this->PC + 1) << 8)) + this->Y;
	this->temp_value   = this->bus.cpu_read(this->temp_address);
	this->PC += 2;
}
...
void CPU::CPU::addressing_indirect()
{
	...
	const std::uint16_t pointer_address =
	  static_cast<std::uint16_t>(pointer_low_byte) | (static_cast<std::uint16_t>(pointer_high_byte) << 8);
	...
	this->temp_address =
	  static_cast<std::uint16_t>(target_low_byte) | (static_cast<std::uint16_t>(target_high_byte) << 8);
}
...
void CPU::CPU::addressing_indirect_x()
{
	...
	this->temp_address =
	  (this->bus.cpu_read(this->temp_address) | (this->bus.cpu_read((this->temp_address + 1) & 0xFF) << 8));
...
void CPU::CPU::addressing_indirect_y()
{
	...
	const uint16_t effective_base_address =
	  static_cast<uint16_t>(effective_base_low) | (static_cast<uint16_t>(effective_base_high) << 8);
