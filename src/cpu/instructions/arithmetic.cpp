...
void CPU::CPU::ADC()
{
	...
	const std::uint16_t result = this->A + this->temp_value + this->get_processor_status_flag(ProcessorStatus::Carry);
...
void CPU::CPU::SBC()
{
	...
	const auto         result =
	  static_cast<std::int16_t>(this->A - this->temp_value - !this->get_processor_status_flag(ProcessorStatus::Carry));
	this->A = result & 0xFF;
