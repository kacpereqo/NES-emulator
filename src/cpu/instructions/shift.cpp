...
void CPU::CPU::ASL_accumulator()
{
	...
	this->set_processor_status_flag(ProcessorStatus::Carry, this->A & 0x80);
...
{
	const std::uint8_t result = (this->temp_value << 1) | this->get_processor_status_flag(ProcessorStatus::Carry);
	this->A                   = result;

	this->set_processor_status_flag(ProcessorStatus::Carry, this->temp_value & 0b1000'0000);
	this->set_processor_status_flag(ProcessorStatus::Zero, this->temp_value == 0);
	this->set_processor_status_flag(ProcessorStatus::Negative, result & 0b1000'0000);
}
...
void CPU::CPU::ROR()
{
	const std::uint8_t result =
	  (this->temp_value >> 1) | (this->get_processor_status_flag(ProcessorStatus::Carry) << 7);
	this->bus.cpu_write(this->temp_address, result);

	this->set_processor_status_flag(ProcessorStatus::Carry, this->temp_value & 0b0000'0001);
	this->set_processor_status_flag(ProcessorStatus::Zero, this->temp_value == 0);
	this->set_processor_status_flag(ProcessorStatus::Negative, result & 0b1000'0000);
}
